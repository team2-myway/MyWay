	...
	public ArrayList getStore_detail(String acc_no, String detail_date) {
		...
		try{
			...
			if(detail_date == null || detail_date.isEmpty() || detail_date.equals("total")) {
				sql = "select m.order_no, m.date, m.total, m.status from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' order by m.order_no desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else if(detail_date.equals("today")) {
				sql = "select m.order_no, m.date, m.total, m.status from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and m.date>curdate() order by m.order_no desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else if(detail_date.equals("month")) {
				sql = "select m.order_no, m.date, m.total, m.status from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and date(m.date)>=date_format(now(), '%Y-%m-01') and date(m.date) <= last_day(now()) order by m.order_no desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else {
				...
				sql = "select m.order_no, m.date, m.total, m.status from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and date(m.date)>=? and date(m.date)<=? order by m.order_no desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
	...
	public StoreDto getStore_total(String acc_no, String detail_date) {
		...
		try{
			...
			if(detail_date.equals("today")) {
				sql = "select sum(m.total) as total from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and m.date>curdate()";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else if(detail_date.equals("month")) {
				sql = "select sum(m.total) as total from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and date(m.date)>=date_format(now(), '%Y-%m-01') and date(m.date) <= last_day(now())";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else if(detail_date == null || detail_date.isEmpty() || detail_date.equals("total")) {
				sql = "select sum(m.total) as total from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료'";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
			} else {
				String[] date = detail_date.split("-");
				String date1 = date[0]+"-"+date[1]+"-"+date[2];
				String date2 = date[3]+"-"+date[4]+"-"+date[5];
				sql = "select sum(m.total) as total from account a INNER JOIN main_order m ON a.account_no=m.account_no where a.account_no=? and status='결제완료' and date(m.date)>=? and date(m.date)<=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, account_no);
				pstmt.setDate(2, java.sql.Date.valueOf(date1));
				pstmt.setDate(3, java.sql.Date.valueOf(date2));
